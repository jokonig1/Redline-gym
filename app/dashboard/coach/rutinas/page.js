...
/**
 * Página de gestión de rutinas predefinidas del coach.
 * Permite crear, editar y eliminar plantillas de rutina con sus ejercicios.
 */
export default function CoachRutinas() {
  ...
  // Modal crear/editar
  const [modal,   setModal]       = useState(null)  // null | { id?, nombre, ejercicios[] }
  ...
  function setNombreModal(val) {
    setModal(m => ({ ...m, nombre: val }))
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-white font-black text-lg">Mis rutinas</h2>
      ...
      {rutinas.length === 0 ? (
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-10 text-center">
          ...
          <div className="text-white font-bold mb-1">Sin rutinas todavía</div>
        ...
        <div className="space-y-3">
          {rutinas.map(r => (
            <div
              key={r.id}
              className="bg-[#141414] border border-white/5 rounded-2xl px-5 py-4"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="text-xs text-zinc-500 hover:text-white px-3 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition-all"
                  >
                    Editar
                  </button>
                  <button
                    onClick={() => setConfirmDel(r)}
                    className="text-xs text-zinc-700 hover:text-red-400 px-3 py-1.5 rounded-lg border border-white/5 hover:border-red-900/30 transition-all"
                  >
                    ✕
                  </button>
                </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-[#141414] border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl flex flex-col max-h-[88dvh]">
            ...
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#1c1c1c] border border-white/5 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-red-600 transition-colors placeholder:text-zinc-700"
                />
              </div>
                ...
                <div className="space-y-2">
                  {modal.ejercicios.map((ej, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <span className="text-[10px] text-zinc-700 w-5 text-center shrink-0">
                        {idx + 1}
                      </span>
                      <input
                        type="text"
                        ...
                        className="flex-1 bg-[#1c1c1c] border border-white/5 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-600 transition-colors placeholder:text-zinc-700"
                      />
                      <button
                        onClick={() => quitarEjercicio(idx)}
                        className="text-zinc-700 hover:text-red-400 transition-colors w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-900/10 text-sm shrink-0"
                      >✕</button>
                    </div>
                  ))}
                </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#141414] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="text-center mb-5">
              ...
              <p className="text-white font-bold">¿Eliminar rutina?</p>
