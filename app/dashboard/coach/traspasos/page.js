    ...
    <div className="max-w-5xl space-y-6">
        ...
        <div className="space-y-3">
              ...
              {[
                ...
                <button
                  ...
                  className={`flex-1 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${
                    modoTemp === key ? 'bg-red-600 text-white' : 'text-zinc-500 hover:text-foreground'
                  }`}
                >
                  {label}
                </button>
                  ...
                  {alumnosFiltradosTemp.length === 0 ? (
                    ...
                    <label
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm cursor-pointer transition-colors ${
                        alumnosSelecTemp.has(a.id) ? 'bg-red-600/10 text-foreground' : 'hover:bg-hover text-foreground'
                      }`}
                    >
                      <input
                        type="checkbox"
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-raised border border-border text-foreground rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-600 placeholder:text-zinc-600"
            />
          </div>

          {errorTemp && (
            <p className="text-xs text-red-400 bg-red-900/20 border border-red-900/30 rounded-lg px-3 py-2">{errorTemp}</p>
          )}
                ...
                <div key={t.id} className="flex items-center justify-between gap-3 bg-raised rounded-lg px-3 py-2.5">
                  ...
                  {vigente && (
                    <button
                      ...
                      className="shrink-0 text-[11px] text-zinc-500 hover:text-red-400 px-2.5 py-1 rounded-lg border border-border hover:border-red-900/30 transition-all"
                    >
                      Cancelar
                    </button>
                  )}
      ...
      <div className="bg-surface border border-border rounded-xl p-5">
        ...
        <div className="space-y-3">
              ...
              <div className="max-h-52 overflow-y-auto rounded-lg border border-border divide-y divide-border">
                {alumnosFiltrados.length === 0 ? (
                  ...
                  <label
                    key={a.id}
                    className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm cursor-pointer transition-colors ${
                      alumnosSelec.has(a.id) ? 'bg-red-600/10 text-foreground' : 'hover:bg-hover text-foreground'
                    }`}
                  >
                    <input
                      type="checkbox"
          ...
          {errorPerm && (
            <p className="text-xs text-red-400 bg-red-900/20 border border-red-900/30 rounded-lg px-3 py-2">{errorPerm}</p>
          )}
