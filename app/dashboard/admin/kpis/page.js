...
function StatCard({ tag, label, value, sub, color = '#ef4444', note }) {
  return (
    <div className="bg-[#141414] border border-white/5 rounded-2xl p-5 flex flex-col gap-1"
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-sm text-zinc-300 truncate max-w-[65%]">{label}</span>
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-end justify-between">
        ...
        <span className="text-[10px] text-zinc-600 bg-white/3 border border-white/5 px-3 py-1.5 rounded-full">
          Actualizado ahora
        </span>
      </div>
      ...
      {/* ── Fila 2: Asistencia + Ocupación (anillos) ── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-5">
          ...
          <div className="flex items-center gap-5">
            <div className="relative shrink-0">
              <Ring pct={asistencia.tasa ?? 0} color="#22c55e" />
            ...
            <div className="space-y-3 flex-1">
              <div>
                ...
                <div className="text-xs text-zinc-500">asistencias confirmadas</div>
              </div>
              <div>
                <div className="text-lg font-black text-red-400">{asistencia.total - asistencia.asistieron}</div>
                <div className="text-xs text-zinc-500">inasistencias</div>
              </div>
              {asistencia.total === 0 && (
                <div className="text-xs text-zinc-700 italic">Sin registros esta semana</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-5">
          ...
          <div className="flex items-center gap-5">
            <div className="relative shrink-0">
              <Ring pct={tasaOcupacion ?? 0} color="#06b6d4" />
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-xl font-black text-white">
            ...
            <div className="space-y-3 flex-1">
              <div>
                ...
                <div className="text-xs text-zinc-500">alumnos activos</div>
              </div>
              <div>
                <div className="text-lg font-black text-zinc-400">{capacidadMax - (alumnos.activos ?? 0)}</div>
                <div className="text-xs text-zinc-500">cupos disponibles</div>
              </div>
              <div className="text-[10px] text-zinc-600">Capacidad máx: {capacidadMax}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-5">
          ...
          <div className="space-y-3.5">
            {porPlan.filter(p => p.count > 0).length === 0 ? (
              <p className="text-xs text-zinc-700 italic">Sin datos</p>
            ) : (
              porPlan
                .filter(p => p.count > 0)
      ...
      <div className="bg-[#141414] border border-white/5 rounded-2xl p-5">
        ...
        {excepciones.total === 0 ? (
          <div className="text-center py-6">
            ...
            <div className="text-sm text-zinc-400 font-medium">Sin cancelaciones ni reagendamientos esta semana</div>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={label} className="bg-white/3 border border-white/5 rounded-xl p-4 text-center"
                style={{ borderTop: `2px solid ${color}40` }}>
                <div className="text-3xl font-black" style={{ color }}>{value}</div>
                <div className="text-xs font-semibold text-zinc-300 mt-2">{label}</div>
        ...
        {excepciones.total > 0 && (
          <div className="mt-4">
            <div className="h-3 bg-white/5 rounded-full overflow-hidden flex">
              <div
                className="h-full bg-red-500/70 rounded-l-full transition-all duration-700"
                style={{ width: `${Math.round(excepciones.cancelaciones / excepciones.total * 100)}%` }}
              />
              <div
                className="h-full bg-amber-400/70 rounded-r-full transition-all duration-700"
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-4 text-center">
          <div className="text-2xl font-black text-white">{coaches}</div>
          <div className="text-xs text-zinc-500 mt-1">Coaches activos</div>
        </div>
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-4 text-center">
          <div className="text-2xl font-black text-white">{alumnos.total}</div>
